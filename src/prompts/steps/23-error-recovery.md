...
Create `audit-reports/` in project root if needed. Save as `audit-reports/23_ERROR_RECOVERY_REPORT_[run-number]_[date]_[time in user's local time].md`, incrementing run number based on existing reports.
