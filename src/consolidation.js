...
export async function generateActionPlan(executionResults, projectDir, { timeout, actionsFile } = {}) {
  if (executionResults.completedCount === 0) {
    info('Skipping action plan — no steps completed');
    return null;
  }

  const filename = actionsFile || ACTIONS_FILENAME;
  ...
  try {
    ...
    if (!result.success || !result.output?.trim()) {
      warn(`Action plan generation failed — ${filename} will not be created. Report is still available.`);
      return null;
    }

    const actionsPath = path.join(projectDir, filename);
    ...
    warn(`Action plan generation error (${err.message}) — ${filename} will not be created.`);
    return null;
  }
}
