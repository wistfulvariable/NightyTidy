...
test/
  ...
  gui-server.test.js       # 58 tests — HTTP server, static files, config, run-command, kill-process, delete-file, heartbeat, log-error, log-path, security headers, traversal, singleton guard, update check, watchdog heartbeat refresh
...
- **GUI server security**: Binds to `127.0.0.1` only. No CORS headers. Body limit 1 MB. Path traversal protection with trailing separator boundary check. Security headers on all responses (HTML, JSON, and error responses). CSP uses `'self'` + `worker-src blob:` (no inline scripts). Frontend heartbeat uses two layers: Web Worker with absolute URL (immune to Chrome tab throttling) + main-thread `setInterval` backup (both run simultaneously). Server watchdog skips heartbeat checks and **refreshes `lastHeartbeat = Date.now()`** when `activeProcesses.size > 0` — the server will NEVER self-terminate while steps are running. The refresh is essential: without it, Chrome-throttled heartbeats during a long fetch would leave `lastHeartbeat` 20s+ stale, and the watchdog tick right after the step ends would kill the server (ECONNREFUSED on next browser fetch → generic "command did not complete" error). The 125-min process safety timeout handles truly stuck processes. When idle, watchdog uses 15s threshold to detect browser gone. **Singleton guard**: lock file in `os.tmpdir()/nightytidy-gui.lock` (PID + URL + port). On startup, checks if existing PID is alive + HTTP-probes the server; if responsive, prints "already running", calls `launchChrome()` to focus the existing window, and exits. Stale locks (dead PID or unresponsive server) are removed automatically. Lock is removed in `cleanup()`. `NIGHTYTIDY_NO_CHROME=1` env var suppresses Chrome launch (used in tests).
